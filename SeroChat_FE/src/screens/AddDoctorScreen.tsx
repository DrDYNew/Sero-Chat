...
const AddDoctorScreen = () => {
  ...
  const loadSpecialties = async () => {
    try {
      const response = await doctorService.adminGetSpecialties();
      if (response.success) {
  ...
  const uploadImage = async (uri: string) => {
    try {
      setUploading(true);
      // Tạm thời tạo doctor với thông tin cơ bản để có ID
      if (!formData.name || !formData.specialtyId) {
        Alert.alert('Lưu ý', 'Vui lòng nhập tên và chọn chuyên khoa trước khi tải ảnh lên');
        return;
      }

      // Tạm lưu URI local cho đến khi submit
      setFormData({ ...formData, imageUrl: uri });
      Alert.alert('Thành công', 'Đã chọn ảnh. Nhấn "Thêm bác sĩ" để hoàn tất.');
    } catch (error) {
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      // Tạo bác sĩ trước
      const response = await doctorService.adminCreateDoctor({
        name: formData.name,
      ...
      if (response.success && response.data) {
        ...
        if (formData.imageUrl) {
          try {
            await doctorService.adminUploadAvatar(doctorId, formData.imageUrl);
          } catch (uploadError) {
        ...
        Alert.alert('Thành công', 'Đã thêm bác sĩ mới', [
  ...
  return (
    <View style={styles.container}>
      ...
      <View style={styles.header}>
        ...
        <Text style={styles.headerTitle}>Thêm Bác sĩ</Text>
        ...
        >
          {loading ? (
            ...
            <>
              ...
              <Text style={styles.submitButtonText}>Thêm bác sĩ</Text>
            </>
          )}
        </TouchableOpacity>
      </View>
