...
import { adminCreateRelaxAsset, adminUploadMedia, adminUploadThumbnail } from '../services/relaxService';

const AddRelaxAssetScreen = () => {
  ...
  const uploadMedia = async (uri: string, type: 'video' | 'audio') => {
    try {
      ...
      if (response.success && response.data?.mediaUrl) {
        ...
        Alert.alert('Thành công', 'Đã upload media lên Cloudinary');
      }
    } catch (error) {
  ...
  const uploadThumbnail = async (uri: string) => {
    try {
      ...
      if (response.success && response.data?.thumbnailUrl) {
        ...
        Alert.alert('Thành công', 'Đã upload thumbnail lên Cloudinary');
      }
    } catch (error) {
  ...
  const handleSubmit = async () => {
    ...
    try {
      setLoading(true);
      const response = await adminCreateRelaxAsset({
      ...
      if (response.success) {
        Alert.alert('Thành công', 'Đã tạo relax asset mới', [
          { text: 'OK', onPress: () => navigation.goBack() }
        ]);
      }
    } catch (error) {
      console.error('Error creating relax asset:', error);
      Alert.alert('Lỗi', 'Không thể tạo relax asset');
    } finally {
      setLoading(false);
    }
  };
  ...
  return (
    <View style={styles.container}>
      ...
      <View style={styles.header}>
        ...
        <Text style={styles.headerTitle}>Thêm Relax Asset</Text>
              ...
              {uploadingMedia ? (
                ...
                <>
                  ...
                  <Text style={styles.uploadButtonText}>
                    {formData.mediaUrl ? 'Đã upload ✓' : 'Chọn file media'}
                  </Text>
                </>
              )}
            </TouchableOpacity>
        ...
        >
          {loading ? (
            ...
            <>
              ...
              <Text style={styles.submitButtonText}>Tạo Relax Asset</Text>
            </>
          )}
        </TouchableOpacity>
      </View>
...
export default AddRelaxAssetScreen;
